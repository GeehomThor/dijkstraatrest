...
public class GithubJerseyClientProxy  implements JerseyClientProxy {
	...
	public List<String> getUserRepos(String user) {
		...
		JsonArray response = client
         .target(String.format("https://api.github.com/users/%s/repos", user))
         .request().get(JsonArray.class);
		
		return response.stream()
			.map(jsonValue -> jsonValue.asJsonObject().getString("name"))
	...
	public List<String> getRepoContributors(String user, String repo) {
		...
		JsonArray response = client
         .target(String.format("https://api.github.com/repos/%s/%s/contributors", user, repo))
         .request().get(JsonArray.class);
		
		return response.stream()
			.map(jsonValue -> jsonValue.asJsonObject().getString("login"))
			.collect(Collectors.toList());
